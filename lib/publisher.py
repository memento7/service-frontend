...
def publish_work(pagedata) :
	""" Function called by multiprocessing.Process
	"""
	#s3 = boto3.resource('s3')
	cprint.okb('UPLOAD ' + pagedata['bucket_name'] + '  ' + pagedata['object_key'])

	s3.Object(pagedata['bucket_name'], pagedata['object_key']).put(
		Body = open(pagedata['file'], 'rb'),
		ACL = 'public-read',
